...
function renameFiles(dir, subjectName) {
  ...
  items.forEach(item => {
    ...
    if (!fs.statSync(oldPath).isDirectory() && item.endsWith('.pdf')) {
      ...
      // Pattern 1: 9395_w22_ms_12.pdf
      let match = item.match(/^(\d+)_([msw]\d{2})_(ms|qp|er|gt|ir|in|ci|pm)_?(\d+)?\.pdf$/i);
      ...
      if (match) {
        ...
        if (sessionInfo) {
          ...
          const newName = subjectName + '-' + sessionInfo.year + '-unit' + paperNum + '-' + sessionInfo.year + '-' + sessionInfo.month + '-' + newType + '.pdf';
      ...
      // Pattern 2: 2013-unit1-2013-06-9395-12-qp.pdf
      match = item.match(/^(\d{4})-unit(\d+)-(\d{4})-(\d{2})-(\d+)-(\d+)-(qp|ms|er|gt|ir|in|ci|pm)\.pdf$/i);
      ...
      // Pattern 3: 2015-unit1-2015-01-WAC01-01-qp.pdf
      match = item.match(/^(\d{4})-unit(\d+)-(\d{4})-(\d{2})-([A-Z]+\d+)-\d+-(qp|ms|er|gt|ir|in|ci|pm)\.pdf$/i);
      
      if (match) {
        const [, year, unit, , month, , type] = match;
        const newName = subjectName + '-' + year + '-unit' + unit + '-' + year + '-' + month + '-' + type + '.pdf';
