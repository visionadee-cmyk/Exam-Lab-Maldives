...
const dir = 'C:/Users/maushaz.MADIHAA/Desktop/Rettey/exam-lab-maldives/exam-lab-mv/public/pdf-pastpaer-q&a/O-Level/ComputerScience-0478';
const subjectName = 'ComputerScience-0478';
...
for (const file of fs.readdirSync(dir)) {
  ...
  // Only rename files belonging to this subject code.
  if (!lower.startsWith('0478_')) continue;

  // Ignore already-custom files that will not pair cleanly.
  if (/_solved\.pdf$/i.test(file)) continue;
  if (/notice\.pdf$/i.test(file)) continue;

  const m = file.match(/^(\d+)_([msw]\d{2})_([a-z]+)(?:_(\d+))?\.pdf$/i);
  if (!m) continue;

  const session = m[2].toLowerCase();
  const type = normalizeType(m[3]);
