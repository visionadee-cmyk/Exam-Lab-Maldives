/**
 * Convert O-Level QP (+ paired MS) PDFs under public/pdf-pastpaer-q&a/O-Level
 * into src/data/papers/*.json with questions and mark-scheme answers.
 *
 * Usage: node scripts/build-olevel-json.cjs [--subject Biology-0610] [--limit N] [--dry-run]
 */
const fs = require('fs');
...
const REPORT_PATH = path.join(ROOT, 'scripts', 'build-olevel-json-report.json');

/** subjectId when filename syllabus differs from folder default (e.g. 7707 in Accounting folder). */
const SYLLABUS_SUBJECT_IDS = {
  accounting: { '0452': 'accounting_igcse', '7707': 'accounting_cambridge_olevel' }
};

function resolveSubjectId(slug, syllabus, fallback) {
  return SYLLABUS_SUBJECT_IDS[slug]?.[syllabus] ?? fallback;
}

const SUBJECT_MAP = {
  'Accounting-0452': { slug: 'accounting', syllabus: '0452', subjectId: 'accounting_igcse' },
  'Biology-0610': { slug: 'biology', syllabus: '0610', subjectId: 'biology_igcse' },
  'BusinessStudies-0450': { slug: 'business', syllabus: '0450', subjectId: 'business_igcse' },
  'Chemistry-0620': { slug: 'chemistry', syllabus: '0620', subjectId: 'chemistry_igcse' },
  'ComputerScience-0478': { slug: 'computer_science', syllabus: '0478', subjectId: 'computer_science_igcse' },
  'Economics-0455': { slug: 'economics', syllabus: '0455', subjectId: 'economics_igcse' },
  'EnglishFirstLanguage-0500': { slug: 'english_first', syllabus: '0500', subjectId: 'english_first_igcse' },
  'EnglishSecondLanguage-0510': { slug: 'english_second', syllabus: '0510', subjectId: 'english_second_igcse' },
  'Mathematics-0580': { slug: 'mathematics', syllabus: '0580', subjectId: 'mathematics_igcse' },
  'Physics-0625': { slug: 'physics', syllabus: '0625', subjectId: 'physics_igcse' },
  'TravelTourism-0471': { slug: 'travel_tourism', syllabus: '0471', subjectId: 'travel_tourism_igcse' }
};
...
function sessionLabel(year, month) {
...
function parseFilename(qpFile, folderName) {
  ...
  const m = base.match(
    /^(.+?)-(\d{4})-unit(\d+)-(\d{4})-(\d{2})(?:-(\d{1,2}))?-(qp|ms)$/i
  );
  if (!m) return null;
  ...
  const subjectId = resolveSubjectId(folderMeta.slug, syllabus, folderMeta.subjectId);
  const meta = { ...folderMeta, syllabus, subjectId };
  ...
  return {
    ...
    paperVariant,
    sessionKey: session,
    session: sessionLabel(year, month),
    meta,
    docType
  };
}

function extractPaperCode(text, fallbackSyllabus) {
  const m = text.match(/\b(\d{4})\/(\d{1,2})\b/);
  if (m) return `${m[1]}/${m[2]}`;
...
function buildMcqJson(qpText, msText, info) {
  ...
  const code = extractPaperCode(qpText, info.meta.syllabus);
...
function buildStructuredJson(qpText, msText, info) {
  ...
  return {
    ...
    code,
    session: info.session,
    title: `Paper ${info.unit} - Question Paper`,
    documentType: 'QP',
