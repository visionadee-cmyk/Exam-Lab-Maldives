...
function renameFiles(dir, subjectName) {
  ...
  items.forEach(item => {
    ...
    if (!fs.statSync(oldPath).isDirectory()) {
      ...
      let match = item.match(/^(\d{4})-unit(\d+)-(\d{4})-(\d{2})-(\d+)-(\d+)-(qp|ms|er|gt|ir|in|ci|pm)\.pdf$/i);
      if (match) {
      ...
      // Pattern 2: 7707_s25_ms_21.pdf
      if (!newName) {
        match = item.match(/^(\d+)_([msw]\d{2})_(ms|qp|er|gt|ir|in|ci|pm)_?(\d+)?\.pdf$/i);
        if (match) {
          ...
          if (sessionInfo) {
            ...
            newName = subjectName + '-' + sessionInfo.year + '-unit' + paperNum + '-' + sessionInfo.year + '-' + sessionInfo.month + '-' + newType + '.pdf';
            newPath = path.join(dir, newName);
          }
        }
      }
